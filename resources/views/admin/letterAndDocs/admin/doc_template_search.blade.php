                ...
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">
                            Search List of Document Template
                        </h3>
                    </div>
                </div>
                <!--begin::Form-->
                <div class="kt-portlet__body">
                    <form action="{{ route('admin.document.template.search') }}" method="post">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <label>&nbsp;</label><br>
                                @can(_permission(\App\Utils\Permissions::ADMIN_LETTER_AND_DOCUMENTS_CREATE))<a class="btn btn-outline-primary pull-right" href="{{ route('admin.document.template.create') }}">+ Create New</a>@endcan
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped custom-table table-nowrap mb-0 kt-datatable">
                            ...
                            <tbody>
                            @if($docTemplate)
                                @foreach($docTemplate as $template)
                                    <tr>
                                        <td>
                                            {{ $template->name ?? '-' }}
                                        </td>
                                        ...
                                        <td class="text-bold text-center">
                                            <button type="button" title="View" class="btn btn-outline-primary" docTemplateId="{{ $template->id }}" id="view-btn" data-toggle="modal" data-target=".bd-example-modal-lg"><i class="fas fa-eye"></i></button>&nbsp;
                                            <a class="btn btn-outline-primary" href="{{ route('admin.document.template.edit', ['id'=>$template->id]) }}"><i class="fas fa-pen"></i></a>&nbsp;
                                            <button class="btn btn-outline-primary active-btn" title="Change Status" docTemplateId="{{ $template->id }}" href="#"><i class="fa fa-exclamation-circle" aria-hidden="true"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
