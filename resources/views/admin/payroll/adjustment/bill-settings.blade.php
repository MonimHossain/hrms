 <!--begin::Form-->
    <form class="kt-form kt-form--label-right" enctype="multipart/form-data" method="POST"
          action="{{ route('mobilebill.adjustment.settings.save') }}">
        {{ csrf_field() }}
        <div class="kt-portlet__body">
            <div class="row">
                <div class="form-group col-md-6">
                    <label class="">Choose default adjustment type for mobile bill</label>
                    <div class="custom-file">
                        <select name="adjustment_type" required class="form-control kt-selectpicker" id="" data-live-search="true">
                            <option value="">Select</option>
                            @foreach($adjustmentType as  $value)
                                <option {{ isset($default) && $default->type_id == $value->id ? 'selected':'' }} value="{{ $value->id }}">{{ $value->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>                
