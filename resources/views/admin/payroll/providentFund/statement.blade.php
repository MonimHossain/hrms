            ...
            <div class="col-md-12">
                ...
                <div class="kt-portlet">
                                ...
                                <div class="col-sm-12">
                                    <div>
                                        <h3 style="float: left;" class="kt-portlet__head-title">
                                           Provident Fund Statement : {{ \Carbon\Carbon::parse($year.'-'.$month)->format('M, Y') }}
                                        </h3>
                    ...
                    <div class="kt-portlet__body">
                        ...
                        <form class="kt-form" action="{{ route('payroll.provident.fund.statement') }}" method="get">
                             @csrf
                            <div class="kt-portlet">
                                <div class="kt-portlet__body">
                                    <div class="row">
                        ...
                        <div class="row">
                            <div class="form-group">
                                {{--@if($generate)
                                    <a href="#" route="{{ route('payroll.provident.fund.statement.generate') }}" smg="Do you want to generate tax!" type="submit" class="btn btn-outline-info confirm">Generate</a>
                                @endif

                                @if($regenerate)
                                    <a href="#" route="{{ route('payroll.provident.fund.statement.regenerate') }}" smg="Do you want to regenerate tax!" type="submit" class="btn btn-outline-primary confirm">Regenerate</a>
                                @endif

                                @if($confirmation)
                                    <a href="#" route="{{ route('payroll.provident.fund.statement.clearance') }}" smg="Do you want to give clearance" type="submit" class="btn btn-outline-warning confirm">Clearance</a>
                                @endif--}}
                            </div>
                        </div>
                        ...
                        <div class="table-responsive">
                            ...
                            <a class="btn btn-success" href="{{ route('payroll.provident.fund.download', ['year'=>$year, 'month'=> $month]) }}">Download</a>

                            @can(_permission(\App\Utils\Permissions::PROVIDENT_FOUND_CLEARANCE_VIEW))
                            <a id="clearance" title="PF Status" data-toggle="modal" data-target="#kt_modal"
                               action="{{ route('payroll.provident.fund.statement.view.clearance', ['year'=>$year, 'month'=> $month]) }}"
                               class="btn btn-success custom-btn globalModal text-white pull-right" style="margin-right: 20px"><i class="fa fa-arrow-down" aria-hidden="true"></i> Clearance  
                            </a>
                            @endcan
                                ...
                                <tbody>
                                    @foreach($history as $row)
                                        <tr>
                                            ...
                                            <td>{{ $row->amount }}</td>
                                            <td>{{ _lang('payroll.pf.status.'.$row->status) }}</td>
                                            <td>{{ $row->remarks }}</td>
                                            <td>
                                                <a href="{{ route('payroll.provident.fund.edit', ['id'=>$row->id]) }}"><i class="flaticon-edit edit-button"></i></a> &nbsp; | &nbsp; 
                                                <a href="{{ route('payroll.provident.fund.delete', ['id'=>$row->id]) }}"><i class="flaticon2-trash delete-button"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
...
@push('js')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: "{{ route('payroll.provident.fund.statement.button.enabled.disabled') }}",
                type: 'post',
                ...
                success: function (data) {
                    ...
                    (data.confirmation) ?  $('.edit-button').show() : $('.edit-button').hide();
                    (data.confirmation) ?  $('.delete-button').show() : $('.delete-button').hide();
                }
            });
        });
