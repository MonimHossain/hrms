    <form class="kt-form kt-form--label-right " action="{{ route('admin.employee.interview.answer.update', ['id'=>$id]) }}" method="POST">
        @csrf
                ...
                <div class="col-lg-12">
                    ...
                    <select class="form-control kt-selectpicker" id="" data-live-search="true" name="question" autocomplete="off">
                        ...
                        @foreach($questions as $question)
                            <option {{ ($row->mst_id == $question->id)? 'selected="selected"':'' }} value="{{ $question->id }}">{{ $loop->iteration }}. {{ $question->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
